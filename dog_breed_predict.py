...
if __name__ == '__main__':
    ...
    ap.add_argument("--folder_path", type=str, default='data/',
    ...
    if folder_path:
        ...
        for file in all_files:
            input_paths.append(os.path.join(folder_path, file))
    else:
    ...
    for path in input_paths:
        ...
        # Độ tin cậy
        conf = np.max(y_pred)
        # Nếu độ tin cậy thấp hơn ngưỡng đặt thì đưa ra là unknown
        if conf >= threshold:
            name = CLASS_NAME[np.argmax(y_pred)]
        print('_________________________________________________________________')
        print(f'Dự đoán: {name}, với độ tin cậy: {round(float(conf) * 100, 2)}%')
        print('_________________________________________________________________')
        # Plotting image to compare
        plt.imshow(origin_img)
        plt.title(f"Predicted: {name}; Confidence: {round(float(conf), 2)}")
        plt.show()
